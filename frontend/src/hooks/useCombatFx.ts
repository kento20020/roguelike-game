...
export function useCombatFx(battle: Battle | null, player: Player | null): CombatFx {
  ...
  const timersRef = useRef<{ enemy?: number; player?: number; crit?: number }>({});
  ...
  useEffect(() => {
    ...
    if (prev) {
      ...
      // SE はログ末尾の kind（=バックエンドが確定させ CombatLog にも表示済みの種別）でのみ分岐する。
      if (lastKind === "hit") playHit();
  ...
  return { enemyShaking, playerShaking, enemyDamage, playerDamage, playerCritFlash };
}
