...
ゲートを抜けた瞬間の「登った」一拍。すぐ次フロアの探索へ繋ぐ短い幕間。
...
### 画面構成
1. **中央に大きな章番号と章名**（serif）。「B{n} ▸ B{n+1}」のように下層から下層へ降りる演出（このゲームは地下カジノ＝降りるほど深い設定。方向は要確認 → §5）。
2. ゲート出目バッジ（`gate_outcome` を日本語化）。被弾時は HP の増減を差分で見せる（例 `-12`、`--danger`）。安全通過なら `--moss` の「無傷」。
3. 進行ドット（B1…B5 のうち到達点を真鍮で点灯）。
...
### 演出
- ゲート真鍮グローが上方向に抜けて、章タイトルがフェードイン（fadeUp）。1.5〜2秒で読めて、CTAは即押せる（待たせない）。
...
2. **死因の一文**（最重要・大きく）: 「{death_floor}層『{章名}』で {死因} に倒れた」。
...
3. **方向の演出** … 「タワーを登る」か「地下へ降りる」か。タイトルは"カジノタワー"だが現デザインは"地下カジノ"。章番号の増減方向と矢印を確定。
