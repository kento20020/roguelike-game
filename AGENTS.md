...
- **画面（UI）は Codex Design で作る。** デザインは Codex Design プロジェクトで `.dc.html` として作成し、`design/` に取り込んでから React コンポーネント化する。レイアウト/見た目を白紙から手書きで起こさない。
...
- PRは `pytest`・`npm run typecheck`・`python scripts/check_docs.py` を通してから（CI必須チェック: balance / check-docs / frontend）
...
- APIエンドポイント仕様 → GDD §25（`/guard`＝受け・`/continue`＝モーダル前進・`/catalog/mods`＝技カタログ・`/profile/upgrades`＝恒久強化 を含む全エンドポイントを記載）
- コンポーネント構成 → GDD §24
- バランス検証フェーズ → GDD §18
