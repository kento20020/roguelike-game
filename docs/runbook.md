...
## 7. 公開前チェックリスト（v1.9 追加）

> 公開時の必須タスク **OPEN-035**（レーティング・LICENSE・データ取り扱い。product_requirements.md §27.2 参照）の実体がこの節。ゲーム内容の合否（operations.md §18.1〜§18.5・§18.8 QA）とは別に、**公開可否**を判断する運用チェックリスト。各項目は他章の詳細を参照するのみで内容を重複させない。

**表記・権利**

- [ ] README に遊び方（起動手順・操作方法の要点）を記載している
- [ ] **リアルマネー賭博ではないことを明記**している（PRD §16.3 公開時注意：チップはゲーム内通貨のみ・リアルマネー無しを README／ストア記載に明記。課金誤認防止に配慮）
- [ ] 年齢レーティングを確認・記載している（カジノ・賭博モチーフのテーマ表現に対する自己判断レーティングを含む）
- [ ] LICENSE と使用素材（画像・音・フォント等）の権利関係を確認している

**既知の問題**

- [ ] 既知の不具合・未解消の制約を列挙している（changelog.md 直近エントリ＋operations.md §21.2 の未解消 OPEN から公開に影響し得るものを抜粋）

**データ・運用**

- [ ] `game.db` のバックアップ手順を確認済み（詳細は §4。自動化は未整備＝OPEN-027 残課題であることも把握）
- [ ] セーブデータ削除／プロファイルリセットの方法を用意している（現状は仕様未定義＝**OPEN-033**。`ProfileRow` は id=1 固定で DB 直接操作以外のリセット手段が無い。公開前に導線の要否を確定する）

**環境・設定**

- [ ] 動作環境を明記している（PRD §27.2：デスクトップブラウザ・min-width 1024px・Chromium系/Firefox 最新2版・Safari ベストエフォート・モバイル非対応）
- [ ] 本番の CORS／`ALLOWED_ORIGINS` を公開先ドメインに設定している（開発既定は Vite dev の2オリジン。`allow_origins=["*"]` のまま公開しない）
- [ ] **冪等性・認証が非スコープのままであることを把握している**（OPEN-026）：全 mutating POST に冪等キーが無く単一プロファイル・認証なし。再送で二重課金・RNGドリフトが起き得ることを許容して公開するか、事前対策するかを判断する
- [ ] workers=1（単一ワーカー）構成で運用している（architecture.md §22.3・§1 の既知の限界）
