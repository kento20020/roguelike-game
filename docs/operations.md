# 運用・検証・未決事項（Operations）
> 元 `game_design_document.md`（v1.2）の §18・§21 を再配置。相互参照の §番号・OPEN-xxx は元GDD準拠。運用インフラ（Alembic/Docker/バックアップ/構造化ログ/監視/CI）は OPEN-027 として未整備＝runbook.md 骨子＋各OPEN参照。

## 18. バランス検証フレームワーク

### 18.1 合格条件

| 指標 | 目標（効果量ベース） | 状態 |
|------|------|:---:|
| **強プレイのクリア率** | **25〜40%**（bot強化状態を固定して測定） | ★確定 |
| ランダムプレイのクリア率 | 1〜5% | 暫定 |
| スキル幅（強−ランダム差） | クリア率差 ≥ X pt（CI下限が0を跨がない＋最小効果量） | 併走 |
| 初手別クリア率差 | ±5pt以内（最大−最小の効果量で判定） | 暫定 |
| 選択感度 | 効果量 ≥ Y pt（p値のみで判定しない） | 併走 |
| 死亡フロア分布 | 終盤集中は許容。最終フロア以外の単一フロアが死亡の Z% を超えない | 暫定 |
| 単一mod寄与 | ±8pt以内（平均寄与のみ） | 暫定 |
| modペア寄与（synergy） | 主要ペアの交互作用が ±W pt 以内 | 暫定（追加・OPEN-023） |
| 初クリアまでの期待ラン数（人間相当bot） | 観測のみ・閾値なし（win-to-progress の体験曲線監視・OPEN-029） | 暫定（追加） |

> **有意性ハック回避（§18.4 Goodhart）**：bot大量試行では n が大きく僅差でも p<0.05・CI0非跨ぎを満たすため、スキル幅・選択感度・初手差は**最小効果量（X/Y pt）を CI下限とともに要求**する（`balance_stats.py` の TOST=同値検定も併用）。
> **死亡フロア分布**：難度ランプのあるパーマデスでは終盤集中が正常。「集中しない」は偽陽性を生むため、終盤集中を許容し「最終フロア以外の単一フロアが過度に偏らない」基準へ置換。
> **クリア率帯の測定条件**：帯（25-40%）は bot の `permanent_upgrades_state` に依存する（+attack等は全戦闘を短縮）。**未強化パネルを基準**とし、フル強化は別帯で管理（bot 強化状態を固定して測る）。
> **strong戦略の版固定**：クリア率帯は strong bot の賢さに対する相対値。`strategy_version` で strong 戦略を凍結し「確定帯は当該版に対してのみ有効」とする（戦略変更時は帯を再検証）。**バランス検証全体が bot の人間代表性に依存する**点は運用上の前提として明記。
> **効果量閾値の事前登録**：X/Y/Z/W の値は**Phase2 開始前に数値で確定して本表へ記入する（事前登録）**。データを見てから閾値を決めるのは効果量版 p-hacking であり禁止。値の決定はデザイナー（人間）、以後の変更は変更履歴で管理。
> **確定帯と guard の不整合（OPEN-018）**：帯 25-40% は **guard 無し bot** での測定値。guard（§8.4）の数値決着と bots.py への実装が済むまで、guard 込み行動空間での帯は未検証＝**Phase1 は OPEN-018/020 の決着後に開始する**。

### 18.2 検証フェーズ

| フェーズ | 目的 | ツール |
|---------|------|------|
| Phase 0 | 実装完了・全テストgreen（既知P0=0） | pytest / TS unit / `loader.validate` |
| Phase 1 | クリア率帯域 | phase12_harness.py |
| Phase 2 | スキル幅・選択感度 | + balance_stats.py |
| Phase 3 | mod単体・ペア影響（FDR） | balance_stats.py(e-BH) + balance_analysis.py(ablation) |
| Phase 4 | 経済・sink ROI・hoarder | balance_analysis.py |
| Phase 5 | 回帰テスト（CI） | GitHub Actions |

> **注記**：Phase0「バグゼロ」は検証不能のため「全テストgreen／既知P0=0」に置換。`fun_metrics.py` は特定Phaseに紐づかない**探索的ツール（合格条件外）**。Phase3 の e-BH は `balance_stats.py`、mod アブレーションは `balance_analysis.py`（旧記述は balance_analysis 単独に見えたため訂正）。guard（§8.4）を正式化する場合は bot 戦略に guard を含めて Phase1-4 を再測定（OPEN-018）。Phase5 の GitHub Actions は現状 **docs-ci（docs整合チェック）のみ稼働**——バランス回帰・pytest のワークフローは未整備（OPEN-027）。

### 18.3 ツール群

> `backend/app/simulation/` の実在ファイルに同期（v1.1）。

| ツール | 機能 | 状態 |
|--------|------|------|
| phase12_harness.py | Bot大量試行（強/random・CRN対応） | 実在 |
| bots.py | botプレイヤー（戦略実装） | 実在 |
| balance_stats.py | e-value統一（BernoulliCS・e-BH・McNemar・TOST・ハザード・レーシング） | 実在 |
| balance_analysis.py | sink ROIアブレーション・hoarder検出 | 実在 |
| balance_report.py | レポート生成 | 実在 |
| fun_metrics.py | 体験指標（面白さ系メトリクス） | 実在 |
| gen_baseline.py | ベースライン生成 | 実在 |
| INTEGRATION.md | 統計手法とフェーズのマッピング | 実在 |
| llm_content_pipeline.py | Generator→Validator→Simulator→Critic | **将来・未実装**（INTEGRATION.md に記載） |

### 18.4 Goodhart回避
- 統計指標は合格条件であり最適化目標ではない
- 死にmodは潰す・壊れmodは残す（非対称運用）
- 帯域内の判断はデザイナーの感性で決める

### 18.5 受け入れ基準（完了の定義 / Definition of Done）

§18.1 はバランスの合格条件。本節は**実装完了の定義**を分けて明文化する（v1.1で追加）。

**機能**
- 新規ラン開始 → 1F〜5F進行 → 5Fゲート通過で `cleared`、HP0で `dead` に遷移する
- ノード選択・戦闘・宝箱（開封/リロール）・回復・ゲートが各phaseで解決できる
- クリア時に恒久強化ポイントを1獲得し割り振れる

**データ（§20.5 機械検証）**
- config/enemies/mods/floors のロード成功
- 全非カオス敵の behaviors weight 合計＝100／全フロアのゲート確率合計＝1.0
- アンロック整合（多親=親2／単親=親1）・存在しないノード参照なし

**API**
- 全エンドポイントが正常系/異常系テストを通過
- phase不整合=409／チップ不足・未対応操作=400／不明session=404（§25.4）

**UI**
- 各phaseに対応画面が表示される／`available_actions` 外の操作は実行不可
- ツリーは row1=画面下・ゲート=画面上で描画（§4・§26.1）

**RNG / テレメトリー**
- 同一seed・同一操作列で同一結果（Python/TS黄金テスト）
- ラン終了時に RunRecord が保存され、`cleared`/`dead` が正しく記録される

> **TS版SFC32の用途**：本番のクライアント計算は無い（原則1「Reactは計算しない」）。TS黄金テストは将来のオフライン/演出用途に向けた**RNGパリティの凍結**であり、演出で使う場合も rng 状態は露出しない演出専用シードに限定する。

---

## 21. 未決事項

> 解消済み項目の確定経緯は巻末「付録: 変更履歴」を参照。本章は現時点の残課題のみを扱う。
> 内部議論ラベル（問E/問G/問J）は管理ID `OPEN-xxx` に振り直した（旧ラベルは括弧で併記）。
> **受け入れ条件（done定義）**：各OPENの方針欄末尾に測定可能な受入条件を付す。データ駆動項目は Phase 判断に委ね、非データ項目（OPEN-011/013/015/018/024 等）は具体的 done を定義する。
> **v1.2 で追加**：OPEN-011〜027 は実装コードとの実差分監査（doc-code drift audit）で洗い出した項目。**v1.3 で OPEN-028/029 を追加**し、OPEN-004/017/018/020/024/025 を改訂（分割後の横断整合レビュー反映）。多くは「コードは実装済みだが GDD 記述が古い/欠落」または「GDD/JSON にもコードにも無い真の欠落」。

### 21.1 確定状態マトリクス

「何が確定で、何が調整中で、何が未決か」の索引。本文inlineの「叩き台/暫定」を集約する。

| 項目 | 状態 | 決定フェーズ／備考 |
|------|------|------------------|
| 戦闘係数（counter 1.0 / heavy 1.8 / ramp_base 5） | **確定** | config.json・実装一致 |
| ゲート確率テーブル（§7.4） | **確定** | floors.json・データは合計1.0（CI検査はOPEN-013） |
| ゲート特殊効果（+40G確定・追加効果未実装・§7.4） | 一部確定 | OPEN-011 |
| 受け（guard・§8.4） | 実装済み・**数値未確定（支配戦略リスク）** | OPEN-018（高・Phase1前） |
| メタ進行の永続（ProfileRow・§20.6） | **確定** | 実装一致 |
| 進行中状態の永続（メモリ・再起動で喪失・§25.1） | 暫定 | OPEN-007 / OPEN-026 |
| experience の正準キー（日本語・§20.7） | 現状維持 | OPEN-012 |
| アンロック連鎖・ツリー形状（§4） | **確定** | floors.json・機械検証済 |
| 強プレイのクリア率帯（25〜40%） | **確定** | §18.1 |
| 敵HP/attack/gold_base（§11.3） | データ駆動調整中（叩き台） | Phase1〜 botで調整 |
| ゴールド期待値（§13.1） | データ駆動調整中（叩き台） | Phase4 |
| 恒久強化の効果値・上限 | データ駆動調整中（構造は確定） | OPEN-002 |
| sinkコスト（攻撃ブースト30G・回復係数） | データ駆動調整中 | OPEN-003 / Phase4 |
| 強敵判定の定義方式（暫定=絶対値4・終盤退化） | 未決 | OPEN-004 / Phase1前 |
| 固有名詞（敵・mod表示名） | 未決 | OPEN-001 |
| フェーズB（スタンス） | 非スコープ（将来） | OPEN-006 |

> §18.1 の暫定/併走指標（ランダムクリア率1-5%・スキル幅・初手別±5pt・選択感度・死亡フロア分布・単一mod寄与±8pt・modペア）は **§18.1 を正**とし、本マトリクスには集約しない（重複管理を避ける）。

### 21.2 未決事項（OPEN）

| ID | 項目 | 内容 | 優先度 | 対応フェーズ | 方針 |
|----|------|------|:------:|:----------:|------|
| OPEN-001 | 固有名詞確定 | 仮名→カジノテーマ名の最終確定（§11.1） | 中 | 実装後半 | §16.2の方向で全敵・全mod名を確定。v1.1では実置換しない |
| OPEN-002 | 恒久強化数値（旧問E） | 各強化項目の効果値・上限の最終調整＋**割り振り導線**（`/upgrade` のスコープ・バンク消化の許可phase・§12.3/§25.2） | 中 | Phase1後 | データ駆動。構造は確定済み。導線は `/api/profile/upgrade` 移設 or 許可phase明記 |
| OPEN-003 | sinkコスト（旧問G） | 攻撃ブースト30G・回復係数含む最終調整 | 低 | Phase4後 | ROI計測後 |
| OPEN-004 | 強敵判定の定義方式（旧問J） | 絶対閾値 diff≥4 は終盤退化（3F 4/8・4F **7/8**・5F **10/10** が強敵＝情報量ゼロ・§11.2）。UI表示・強敵報酬・宝箱確率・統計に波及 | 中 | Phase1前に暫定固定 | 「値」でなく**定義方式**（フロア相対・上位n体等）を再設計。暫定は絶対値4のまま。受入=定義方式の確定＋config反映 |
| OPEN-005 | 攻撃ブースト効果量 | +100%が適正か | 低 | Phase4 | データ駆動。叩き台+100% |
| OPEN-006 | フェーズB | スタンスシステム設計 | 低 | v1.x以降 | A案検証完了後（攻撃ブーストと計算経路共有） |
| OPEN-007 | セーブ/ロード・進行中状態の永続 | ブラウザ離脱＋**サーバ再起動での喪失**（`session_store` はメモリ・単一プロセス） | 中 | 実装中盤 | GameState 全フィールド（rng含む）のシリアライズ仕様を定めDB/Redisへ。**session 上限（LRU破棄）・`DELETE /run/{sid}`（放棄）も本件で扱う**。受入=再起動を跨いで `GET /run/{id}` が成立 |
| OPEN-008 | 演出・SE/BGM | オーディオ仕様 | 低 | 実装後期 | — |
| OPEN-009 | アクセシビリティ | 色覚対応・キーボード操作 | 低 | 実装後期 | — |
| OPEN-010 | 勝利優先時のHP | 同時死亡・勝利優先でHP0生存になる挙動。HP0で探索復帰→回復sink/次ノード/ゲートの扱いが曖昧＝**不変条件（HP0=ゲームオーバー）を壊す**公平性問題 | **高** | **実装前** | **コード変更要**。推奨=勝利時 `hp = max(1, hp)`。受入=HP0生存が発生しない |
| OPEN-011 | ゲート特殊効果 | 特殊の追加効果（現状+40Gのみ・「次フロア強化等」未実装）（§7.4） | 低 | 実装後期 | 効果実装なら受入=効果定義＋発生率検証。当面は +40G 確定 |
| OPEN-012 | experience の enum 化 | 日本語→romaji 正準キー（grind/gamble/race/dodge/chaos）（§20.7） | 中 | 実装中盤 | 統計キー・labels.ts・API を一括変更。受入=日本語が API/統計キーに出ない |
| OPEN-013 | データCI検査の拡充 | gate合計1.0・kind整合・5F経路≥2・上限Lv=21・snapshot非出力（§20.5） | 中 | Phase0〜 | `loader.validate`/テストへ追加。受入=各検査が赤/緑を返す |
| OPEN-014 | 体験タイプ最早出現の制約化 | 段階導入をCI検証（ずれ2F/カオス3F〜）（§4.4/§9.1） | 低 | Phase1後 | §9.1 制約をCIへ。受入=違反プールが fail |
| OPEN-015 | L3情報の state別マスク | 未インタラクトノードの name/max_hp 非送出（§5） | 中 | 実装中盤 | `_node_view` を state別マスク化。受入=available で name/max_hp 非出力 |
| OPEN-016 | ゲート保証の削減0停止 | 大ダメ0%到達後の重ねがけ拒否（§7.4） | 低 | Phase4 | `use_sink` に停止条件＋action非提示。受入=削減0で400 |
| OPEN-017 | 宝箱リロールの判断材料/廃止 | 中身非公開で期待値改善0（Phase4判断）。**1F確定宝箱（fixed_mod）のリロール30G空費は分離して先行対応**（§7.2/§14.2・学習契約§3③違反） | 低（1F分離分は**高・実装前半**） | Phase4／1F分は実装前半 | 一般はヒント付与 or 廃止。1F分は `available_actions` から reroll 非提示（サーバ側1条件） |
| OPEN-018 | guard の正式化・数値是正 | 現数値（deal0.5/incoming0.25・無コスト・回数無制限）は非ランプ敵（ロスター約7割）に対し**総被ダメ半減の支配戦略**になり得る。体験タイプの差別化が潰れ、§18 の全計測（帯25-40%は guard 無し測定）の前提が崩れている（§8.4/§18.1） | **高** | **Phase1 前必須** | 数値是正（コスト付与 or 係数圧縮 0.6/0.5 等・データ駆動）→ bots.py へ guard 実装 → 再測定。**決着まで Phase1 を開始しない**。受入=bot 行動空間が UI と一致 |
| OPEN-019 | -sinkコスト下限 | scout無料化（0G）防止（§13.2） | 低 | Phase4 | `max(1,…)` クランプ。受入=最安sink≥1G |
| OPEN-020 | heal sink のターン消費 | battle中healで敵が反応するか（§13.2）。**guard と併用すると非ランプ敵戦の敗北条件が実質チップ切れに退化** | **高** | **Phase1 前必須（OPEN-018 と束ねて決着）** | 「1ターン消費」案を推奨。受入=heal後に敵ロール発火 |
| OPEN-021 | 攻撃ブースト異常系 | evade巻戻し丸損・pending中二重課金（§13.2/§25.4） | 低 | Phase4 | 持越し/返金＋pending中400。受入=二重課金不可 |
| OPEN-022 | 反射チュートリアル確実化 | 1Fで反射の取得/発火を保証（§14.2） | 低 | 実装後半 | 必須経路配置 or A必須経由。受入=1F内で取得&発火 |
| OPEN-023 | modペア交互作用ゲート | synergy寄与の統計ゲート（§18.1） | 中 | Phase3 | `balance_analysis` アブレーションをペア拡張（e-BH） |
| OPEN-024 | RunRecord data_version | 版混在の統計汚染防止（§19.1） | 高 | 実装中盤（**Phase1 の初回データ取得前**） | 4JSONハッシュ追加＋版フィルタ。**`strategy_version`（bot戦略版・human は null）も同一マイグレーションで追加**。受入=版別集計可 |
| OPEN-025 | sink use_count / gate_results / guard記録 | sink別使用回数・per-floorゲート結果・**guard使用回数**が無く ROI/寄与を逆算不能（§19.1） | 中 | Phase4前（Phase1 の初回データ取得前が望ましい） | 回数・ゲート結果・guard回数（`actions:{attack,guard}`）を記録。受入=ROI(回数,効果)算出可 |
| OPEN-026 | 認証・冪等性・stats scope | 進行中DB永続・冪等キー・stats/history のスコープ（§25） | 中 | 実装後半 | 単一プレイヤーは非スコープ明記（§0.4）＋ stats scope＋全mutating POSTに冪等キー |
| OPEN-027 | 運用基盤の整備 | Alembic・Docker・game.dbバックアップ・構造化ログ+req_id・.env.example・CI・`/health`・RUNBOOK/ONBOARDING（§22/§23） | 中 | 実装中盤〜後半 | **GDD外の別タスク**。`docs/RUNBOOK.md` 等へ最小構成を整備。受入=別環境で再現可能な起動/デプロイ/バックアップ手順 |
| OPEN-028 | カオスの ramp_hit 空砲 | カオスは increment=0 のため ramp_hit 被ダメ常時5。ramp偏重（最大85%）を引いた個体はほぼ無害＝実効強度の分散が極端（§9.2） | 低 | Phase1後 | 「意図的な当たり枠」と明記 or カオス専用の小 increment 付与（データ変更のみ）。受入=§9.2 に評価を明文化 |
| OPEN-029 | 初クリアまでの体験曲線 | win-to-progress は初クリアまでメタ進行ゼロ（§2.1）。人間相当の弱めbotでの初クリア期待ラン数が長すぎると離脱点になる | 低 | Phase2 | §18.1 の観測指標で監視（閾値なし）。超過が見えた場合のみ救済（到達フロア別微小ポイント等）を検討 |
