...
> **確定帯と guard の不整合（OPEN-018）**：帯 25-40% は **guard 無し bot** での測定値だった。v1.8 でジャストガード再設計と bots.py（strong-v2）への受け方策実装が完了し、guard 込み行動空間での測定が可能になった。**Phase1 は guard 数値（軽減率・減衰係数）の確定後に開始する**。
...
| 受け（guard・§8.4） | **ジャストガード再設計済み（v1.8）**・数値は叩き台 | OPEN-018（支配戦略はルールで排除済み・数値確定のみ残） |
...
| OPEN-018 | guard の正式化・数値是正 | 旧数値（deal0.5/incoming0.25・無コスト・回数無制限）は非ランプ敵（ロスター約7割）に対し**総被ダメ半減の支配戦略**になり得た | 中 | **大半解消（v1.8）** | **ジャストガード＋重ねがけ減衰へ再設計済み**（軽減率が敵の実際の行動に連動 heavy90%/counter50%/ramp50%/空振り0%・使用毎に軽減量半減・戦闘毎リセット）。盲guardの期待値が消え支配戦略をルールで排除（§8.4・`docs/proposals/guard_redesign.md`）。bots.py strong-v2 へ受け方策を実装し受入条件「bot行動空間=UI一致」達成。**残: 軽減率・減衰係数の数値確定（感度分析→デザイナー）。確定まで Phase1 を開始しない** |
