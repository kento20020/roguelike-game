...
> **v1.12**（Phase3 実施＝mod ablation・mikiri/yomi 是正。現行版の正本は [changelog.md](changelog.md) 先頭エントリ）。本書は従来1ファイルに集約していた設計書を役割別に分割した**目次（インデックス）**です。各内容は下記の分割ファイルが正本。相互参照の §番号・OPEN-xxx は分割前の番号を維持しています。
...
| [operations.md](operations.md) | バランス検証・未決事項（OPEN-001〜047）・運用方針 | §18, §21 |
