...
- **v1.11**（2026-07-11）：**Phase2（スキル幅・選択感度）実施＋効果量閾値の事前登録**（[reports/2026-07-11_phase2_report.md](reports/2026-07-11_phase2_report.md)）。
  ...
  - **選択感度は mid パネルで実証**：初手 L/M/R で 6.3pt・FDR有意（maxed では天井効果で検出されず＝測定パネルを mid へ改修推奨）。同時に **mid の初手別±5pt 違反（L 31.1%＞M 24.8%/R 24.9%）を検出＝OPEN-045 起票**。死亡フロア集中は PASS（最終以外最大 4F 33.8%）。
  - **OPEN-046 起票**：strong bot が scout・ゲート保証・宝箱リロールを未使用（3sink の bot 検証が不可能＝人間代表性の穴）。

- **v1.10**（2026-07-11）：**OPEN-030 解消＝難易度カーブの数値調整（クラスB・PR #33）とクリア率帯の測定基準改訂（クラスS）**。
  - **G1セット採用**：balance_model.md の構造分析（ゲート累計被ダメ≈56%・tier×row 乗算による終盤敵HP暴発・回復経済の目詰まり）に基づき、候補7案＋微調整3案を N=1000（CRN）で比較して選定。`base_attack` 15→16／`enemy_hp_tier_factor` 0.15→0.10／`hp_per_row` 0.10→0.05／`gate_damage` 10/25%→**8/18%**／回復 node 15/30→**20/45**・sink base 20/40→**15/30**・係数上限 2.0→1.8。深度カーブ（2/3/4/5/6）は据え置きで達成（深度圧縮単独は収入減で逆効果と実測）。
  - **帯の測定基準を mid パネルへ改訂（§18.1・デザイナー決定）**：旧基準「未強化パネルで 25〜40%」は現行の恒久強化幅と両立不可能なことが実験で判明（base を帯に入れると maxed が形骸化）。**mid（〜6クリア相当）で 31.1% CI[28.3,34.0]＝帯内達成**。base 1.2%（初クリア現実化）・maxed 82.5%・random 0% は別帯/残課題として観測継続。未強化基準の復活は恒久強化の効果値圧縮（OPEN-002）とセットで再検討。
  - **追随**：GDD数表（§7.3/§7.4/§11.3/§12.1/§12.2/§13.2）・テスト8件・`gen_baseline` 再生成（base1/mid35/maxed92 per 120）・pytest 243 green。**Phase1 開始可能**。残課題: random 1〜5%（暫定）・チップ収支の再定義（§13.1・OPEN-003/Phase4）。
